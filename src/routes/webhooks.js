...
const assignTradeToStaff = async (tradePayload) => {
  try {
    ...
    if (eligibleStaff.length === 0) {
      console.log('No Paxful eligible staff found. Saving trade to manual unassigned collection.');
...
router.post('/trade/mark', async (req, res) => {
  ...
  try {
    ...
    const staffSnapshot = await admin.firestore().collection('Traineestaff').get();
    let staffToUpdate;
    ...
    const staffRef = admin.firestore().collection('Traineestaff').doc(staffToUpdate.docId);
    const staffDoc = await staffRef.get();
